...
import { addlicenseetest } from '@/app/lib/actions';
...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    const form = new IncomingForm({
      uploadDir: path.join(process.cwd(), 'uploads'), // Absolute path to the uploads directory
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        await addlicenseetest(licenseData);
